...
export default async function PrintNotaPage({
  ...
  const invoiceCode =
    (data as any).invoiceCode || `TRX-${data.id.slice(0, 8)}`;
    ...
    <>
      ...
      <html lang="id">
        <head>
          ...
          <title>Nota {invoiceCode} — D'fix</title>
          <style>{`
            * {
              margin: 0;
              padding: 0;
            ...
            @media print {
              html, body {
                ...
                margin: 0;
                padding: 0;
                /* Prevent browser from adding its own header/footer (URL, date, page number) */
                -webkit-print-color-adjust: exact;
                print-color-adjust: exact;
              }

              .nota {
                /* Use full printable width — works for A4 and thermal */
                max-width: 100%;
                padding: 0;
                margin: 0;
              }

              /* Suppress browser chrome headers/footers */
              @page {
                margin: 12mm 10mm;
                size: auto;
              }
          ...
          <div className="nota">
            ...
            <div className="header">
              ...
              <p className="invoice-sub">
                {invoiceCode} &nbsp;·&nbsp; {formatDate(data.createdAt)}
              </p>
            </div>
            ...
            {(data as any).category && (
              <div className="row">
                <span className="label" style={{ color: "#999", fontSize: 11 }}>
                  Kategori
                </span>
                <span className="value" style={{ color: "#999", fontSize: 11 }}>
                  {(data as any).category}
                </span>
              </div>
            )}
            ...
            <div className="row">
              ...
              <span
                className="value"
                style={{ color: sisa > 0 ? "#e53935" : "#16a34a" }}
              >
                {formatIDR(sisa)}
              </span>
            </div>
            ...
            <div className="row">
              ...
              <span
                ...
                style={{
                  color:
                    (data as any).paymentStatus === "LUNAS"
                      ? "#16a34a"
                      : "#2563eb",
                }}
              >
