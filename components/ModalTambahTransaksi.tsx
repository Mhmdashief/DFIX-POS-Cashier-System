...
export function ModalTambahTransaksi({ isOpen, onClose, onSuccess }: ModalProps) {
    ...
    if (isOpen) {
      const fetchRefData = async () => {
        ...
        const uniqueKategori = Array.from(
          new Set(resLayanan?.map((item: any) => item.category).filter(Boolean))
        ) as string[];

        setOptions({
          pelanggan: resPelanggan || [],
          jasa: resLayanan || [],
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const totalAmount = parseInt(formData.estimasi_harga) || 0;
      const dpAmount = parseInt(formData.dp_dibayar) || 0;
      
      let paymentStatus = "Belum Bayar";
      if (dpAmount >= totalAmount && totalAmount > 0) {
        paymentStatus = "Lunas";
      } else if (dpAmount > 0) {
        paymentStatus = "DP Bayar";
      }

      const result = await addTransactionAction({
        customerName: formData.nama_pelanggan,
        serviceName: formData.jenis_jasa,
        ...
        <form onSubmit={handleSave} className="px-8 pb-8 space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              {/* Menggunakan Input + Datalist agar bisa diketik */}
              <input
                required
                list="customer-list"
                placeholder="Ketik nama pelanggan..."
                className="w-full pl-16 pr-10 py-3.5 bg-white border border-zinc-200 rounded-2xl text-[13px] outline-none focus:border-[#2D4F53] transition-all"
                value={formData.nama_pelanggan}
                onChange={(e) => setFormData({ ...formData, nama_pelanggan: e.target.value })}
              />

              {/* List opsi yang akan muncul saat diketik */}
            ...
            <div className="relative">
              ...
              >
                ...
                {options.jasa.map((j, i) => (
                  <option key={i} value={j.name}>{j.name}</option>
                ))}
              </select>
